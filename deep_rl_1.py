...
for iteration in xrange(n_iter):
    ...
    thetas = np.random.randn(batch_size, dim_theta) * theta_std + theta_mean
    ...
    elite_thetas_a = np.array(elite_thetas)
    theta_mean = elite_thetas_a.mean(axis=0)
    theta_std = elite_thetas_a.std(axis=0)
    print "iteration %i. mean f: %8.3g. max f: %8.3g" % (
